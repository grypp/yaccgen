...
void PrintTraversal::traverse_call(FunctionCall *node) {
	...
	if (!node->args.empty()) {
		...
		if (((*i)->etype == ET_BinaryExpr) && ((static_cast<BinaryExpr*>(*i))->op() == BO_Comma)) {
			out << "(";
			(*i)->accept(this);
			out << ")";
		} else (*i)->accept(this);
...
void PrintTraversal::traverse_sizeof(SizeofExpr *node) {
	out << "sizeof(";
	if (node->sizeofType) node->sizeofType->printType(out, NULL, true, 0);
...
void PrintTraversal::traverse_expression(ExpressionStemnt *node) {
	...
	for (LabelVector::const_iterator i = node->labels.begin(); i != node->labels.end(); ++i)
		traverse_label(*i);
...
void PrintTraversal::traverse_do_while(DoWhileStemnt *node) {
	...
	if (!node->block->isBlock()) out << std::endl;
