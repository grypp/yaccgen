...
static uint calcHash(const std::string& str) {
	uint hsh = 0, c;
...
void HashTbl::Delete(SymEntry *entry) {
	int j = calcHash(entry->name) % tsize;
...
void HashTbl::Show(std::ostream& out) const {
	out << "HashTab:  #entries: " << nent << "  size: " << tsize << std::endl;
...
ScopeTbl::ScopeTbl(ScopeTbl *mom /* =NULL */, int initSize /* = INIT_CHILD_SIZE */) {
	htab = NULL;
	nchild = 0;
	size = initSize;
	...
	if (mom) level = mom->level + 1;
	else level = EXTERN_SCOPE;
}
...
void ScopeTbl::PostOrderTblDelete() {
	if (this) {
...
ScopeTbl::Lookup(const std::string& sym) {
	...
	if (this && htab) ret = htab->Lookup(sym);
...
ScopeTbl::Find(const std::string& sym) {
	...
	if (this) {
		...
		if (htab) ret = htab->Lookup(sym);

		if (!ret) ret = parent->Find(sym);
...
bool ScopeTbl::ChildInsert(ScopeTbl *kid) {
	if (nchild >= size) {
...
void ScopeTbl::ShowScopeId(std::ostream& out) const {
	if (parent) {
...
void ScopeTbl::Show(std::ostream& out) const {
#ifdef DEBUG_SYMBOL_TBL
	out << "\nScopeTab (" << (void*) this
	<< "): (parent: " << (void*) parent << ")\n";
#endif

	out << "level: " << level << "  nsyms: " << nsyms << "  nchild: " << nchild << std::endl;

	out << "Scope Id: ";
	ShowScopeId(out);
	out << std::endl;

	if (htab) htab->Show(out);
	else out << "HashTab: NULL\n";
...
SymTbl::Lookup(const std::string& nme) {
	...
	if (current) ret = current->Find(nme);
...
SymTbl::Insert(SymEntry *entry) {
	...
	if (!current) return (SymEntry*) NULL;

	while (clevel > current->level) {
		ScopeTbl *child = new ScopeTbl(current);

		if (!child || !current->ChildInsert(child)) return NULL;
...
bool SymTbl::InsertAt(SymEntry *entry, int level) {
	ScopeTbl *scp;
	...
	*gProject->Parse_TOS->yyerrstream
	...
	while ((clevel > current->level) && (clevel >= level)) {
		...
		if (!child || !current->ChildInsert(child)) return false;
	...
	if (scp) return (scp->Insert(entry) != NULL);
	else return false;
}

int SymTbl::EnterScope() {
	reEnter = NULL;
	return ++clevel;
}

int SymTbl::ReEnterScope() {
	if (reEnter) current = reEnter;

	return ++clevel;
}

void SymTbl::ExitScope(bool _reEnter /* =false */) {
	clevel--;

	assert(current != NULL);

	if (_reEnter) reEnter = current;

	if (current->level > clevel) current = current->parent;
	else reEnter = NULL;
}

void SymTbl::Show(std::ostream& out) const {
	out << "\n-------------\nSymbol Table:\n";
...
void Context::ExitScope(bool reEnter /* =false */) {
#ifdef  PRINT_LEVEL
	Indent();
...
void Context::ExitScopes(int newlev) {
	if (newlev < EXTERN_SCOPE) newlev = EXTERN_SCOPE;
...
operator<<(std::ostream& out, const Symbol& sym) {
	...
	if (sym.entry && Project::gDebug) {
		...
		for (const SymEntry *curr = htab->tab[id1]; curr != sym.entry; ++id2) {
			curr = curr->next;
			assert(curr);
			// the symbol have to be in this list!
		}
		out << "$" << id1 << "_" << id2 << "$";
